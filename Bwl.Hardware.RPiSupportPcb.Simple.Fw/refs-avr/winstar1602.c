...
void lcd_string_to_line(char* line, int line_number)
{
	for (int i=0; i<LCD_LINE_LENGTH; i++)
	{
		if(line_number==1)lcd_line_1[i]=line[i];
		if(line_number==2)lcd_line_2[i]=line[i];
	}
}
...
void _lcd_write_half_byte(char rs, unsigned char byte)
{
	pin_set_out(DISPLAY_RS,rs);
	pin_set_out(DISPLAY_E,0);
	pin_set_out(DISPLAY_D7,byte&8);//d3
	pin_set_out(DISPLAY_D6,byte&4);
	pin_set_out(DISPLAY_D5,byte&2);
	pin_set_out(DISPLAY_D4,byte&1);
...
{
	_lcd_write_half_byte(rs, byte>>4);
	if(repeatfirst)_lcd_write_half_byte(rs, byte>>4);
	_lcd_write_half_byte(rs, byte);
	var_delay_us(LCD_DELAY_SYMBOLS);
}

void lcd_setup()
{
	pin_set_dir(DISPLAY_RS,1);
	pin_set_dir(DISPLAY_E,1);
	pin_set_dir(DISPLAY_D7,1);
	pin_set_dir(DISPLAY_D6,1);
	pin_set_dir(DISPLAY_D5,1);
	pin_set_dir(DISPLAY_D4,1);
	_lcd_write_half_byte(0,0);var_delay_us(LCD_DELAY_SYMBOLS*10);
	_lcd_write_half_byte(0,0);var_delay_us(LCD_DELAY_SYMBOLS*10);
	_lcd_write_half_byte(0,0);var_delay_us(LCD_DELAY_SYMBOLS*10);
    _lcd_write_half_byte(0,0);var_delay_us(LCD_DELAY_SYMBOLS*10);
    _lcd_write_half_byte(0,0);var_delay_us(LCD_DELAY_SYMBOLS*10);
